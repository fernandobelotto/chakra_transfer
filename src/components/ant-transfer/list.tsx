...
function getEnabledItemKeys<RecordType extends KeyWiseTransferItem>(
  items: RecordType[]
) {
...
> {
  ...
  getCheckStatus(filteredItems: RecordType[]) {
    ...
    if (
      filteredItems.every(
        (item) => checkedKeysMap.has(item.key) || !!item.disabled
      )
    ) {
      return "all";
    }
  ...
  renderListBody = (
    ...
    if (!customize) {
      bodyContent = (
        <DefaultListBody ref={this.defaultListBodyRef} {...props} />
      );
    }
    return {
      customize,
  ...
  getListBody(
    ...
    const getNotFoundContent = () => {
      ...
      return Array.isArray(notFoundContent)
        ? notFoundContent[contentIndex]
        : notFoundContent;
    };
  ...
  render() {
    ...
    const listFooter = footerDom ? (
      <div className={`${prefixCls}-footer`}>{footerDom}</div>
    ) : null;
          ...
          ? {
              ...
              onClick: () => {
                const pageKeys = getEnabledItemKeys(
                  (this.defaultListBodyRef.current?.getItems() || []).map(
                    (entity) => entity.item
                  )
                );
      ...
      items = [
        ...
        pagination
          ? {
              ...
              onClick: () => {
                const pageItems =
                  this.defaultListBodyRef.current?.getItems() || [];
                onItemSelectAll(
