...
export interface TransferListBodyProps<RecordType>
  ...
  static getDerivedStateFromProps<T>(
    ...
    if (mergedPagination) {
      ...
      const maxPageCount = Math.ceil(
        filteredRenderItems.length / mergedPagination.pageSize
      );
  ...
  render() {
    ...
    return (
      <>
        ...
        >
          {this.getItems().map(
            ({ renderedEl, renderedText, item }: RenderedItem<RecordType>) => {
              const { disabled } = item;
              const checked = selectedKeys.includes(item.key);
